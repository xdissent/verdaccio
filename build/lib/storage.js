...
const getDefaultMetadata = function (name) {
  const pkgMetadata = {
    name,
    versions: {},
    'dist-tags': {},
...
class Storage {
   ...
   * Tags a package version with a provided tag
   Used storages: local (write)
   */
  replaceTags(name, tagHash, callback) {
    ...
    _async2.default.map(upLinks, (upLink, cb) => {
      ...
      upLink.getRemoteMetadata(name, _options, (err, upLinkResponse, eTag) => {
        ...
        if (err || !upLinkResponse) {
          // $FlowFixMe
          return cb(null, [err || _utils.ErrorCode.get500('no data')]);
        }
      ...
      if (!exists) {
        return callback(_utils.ErrorCode.get404('no such package available'), null, upLinksErrors);
      }
