...
class ProxyStorage {
  ...
  request(options, cb) {
    ...
    if (this._statusCheck() === false) {
      ...
      process.nextTick(function () {
        if (cb) {
          cb(_utils.ErrorCode.get500('uplink is offline'));
        }
        // $FlowFixMe
        streamRead.emit('error', (0, _httpErrors2.default)('uplink is offline'));
      });
      // $FlowFixMe
  ...
  _setAuth(headers) {
    ...
    if (!_lodash2.default.isObject(this.config.auth)) {
  ...
  getRemoteMetadata(name, options, callback) {
    ...
    this.request({
      ...
      if (res.statusCode === 404) {
        return callback(_utils.ErrorCode.get404('package doesn\'t exist on uplink'));
      }
      if (!(res.statusCode >= 200 && res.statusCode < 300)) {
        const error = (0, _httpErrors2.default)(500, `bad status code: ${res.statusCode}`);
        // $FlowFixMe
  ...
  fetchTarball(url) {
    ...
    readStream.on('response', function (res) {
      if (res.statusCode === 404) {
        return stream.emit('error', _utils.ErrorCode.get404('file doesn\'t exist on uplink'));
      }
      if (!(res.statusCode >= 200 && res.statusCode < 300)) {
        return stream.emit('error', (0, _httpErrors2.default)(500, 'bad uplink status code: ' + res.statusCode));
      }
    ...
    readStream.on('end', function (data) {
      ...
      if (expected_length && current_length != expected_length) {
        stream.emit('error', (0, _httpErrors2.default)(500, 'content length mismatch'));
      }
    });
    return stream;
  }
  ...
  search(options) {
    ...
    requestStream.on('response', res => {
      if (!String(res.statusCode).match(/^2\d\d$/)) {
        return transformStream.emit('error', (0, _httpErrors2.default)(500, `bad status code ${res.statusCode} from uplink`));
      }
