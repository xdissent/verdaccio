  ...
  constructor(config) {
    ...
    for (let user in self.users) {
      if (Object.prototype.hasOwnProperty.call(self.users, user)) {
        ...
        assert(typeof self.users[user].password === 'string' && self.users[user].password.match(/^[a-f0-9]{40}$/), 'CONFIG: wrong password format for user: ' + user + ', sha1 expected');
      }
    }
    ...
    for (let uplink in self.uplinks) {
      if (Object.prototype.hasOwnProperty.call(self.uplinks, uplink)) {
        ...
        assert(typeof self.uplinks[uplink].url === 'string', 'CONFIG: wrong url format for uplink: ' + uplink);
    ...
    for (let pkg in self.packages) {
      if (Object.prototype.hasOwnProperty.call(self.packages, pkg)) {
        assert(typeof self.packages[pkg] === 'object' && !Array.isArray(self.packages[pkg]), 'CONFIG: bad "' + pkg + '" package description (object expected)');
