...
exports.default = function (route, auth) {
  ...
  route.put('/-/user/:org_couchdb_user/:_rev?/:revision?', function (req, res, next) {
    let token = req.body.name && req.body.password ? auth.aes_encrypt(new Buffer(req.body.name + ':' + req.body.password)).toString('base64') : undefined;
      ...
      auth.add_user(req.body.name, req.body.password, function (err, user) {
        if (err) {
          if (err.status >= 400 && err.status < 500) {
            ...
            return next(_utils.ErrorCode.getCode(err.status, err.message) || _utils.ErrorCode.get409(err.message));
          }
          return next(err);
        }
